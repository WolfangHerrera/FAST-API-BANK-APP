...
async def update_account_balance(customer_dni: int, new_account_balance: float) -> Customer:
    result = await db.accounts.update_one({'dni': customer_dni}, {"$set": {"balance": new_account_balance}})
    if result.modified_count == 0:
        return None

    updated_account_balance = await db.accounts.find_one({"dni": customer_dni})
